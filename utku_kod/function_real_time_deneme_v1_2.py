...
def run_vo(vo):
    global rt_frame_data_list, vo_frame_time, vo_pos

    print('HERE baş', len(rt_frame_data_list))  
    
    while not exit_event_vo.is_set():
        if rt_frame_data_list:
            print('here', len(rt_frame_data_list))
            frame_time, pos_ = vo.run(rt_frame_data_list[0], rt_timelist_zed[0], len(rt_frame_data_list))
            rt_frame_data_list.pop(0)
            rt_timelist_zed.pop(0)
            vo_frame_time.append(frame_time)
            vo_pos.append(pos_)
    
vo_frame_time, vo_pos = [], []
    
def run_main(zed_cam_id):
    global Data_list, frame_data_list, time_list, timelist_zed, rt_frame_data_list, rt_timelist_zed, vo_frame_time, vo_pos
    ...
    while len(rt_frame_data_list) == 0:
        pass
    
    vo = VO(P0, P1, (rt_frame_data_list[0], rt_timelist_zed[0]), filter_match_distance=0.2)
    rt_frame_data_list.pop(0)
    rt_timelist_zed.pop(0)
    
    calculate_vo = True # False diyip aşağıda qvo hariç olanı çalıştırmazsanız eskisi gibi çalışır
    if calculate_vo:
        vo_thread = threading.Thread(target=run_vo, args=(vo,))
        vo_thread.start()
              
          
    while not exit_event.is_set():
        entry= input('Enter qa(to not save) or qw (to save) to exit:')
        print(len(rt_frame_data_list))
        if entry == 'qw':
            ser.close()
            exit_event.set()
            exit_event_vo.set()
            ...
            return entry, Data_list, frame_data_list,time_list, timelist_zed, ""
        ...
        elif entry == 'qvo': # keep calculating VO
            ...
            while rt_frame_data_list:
                time.sleep(3)
                print(f"Total Frame:{len(frame_data_list)}, More Frames to Calculate:{len(rt_frame_data_list)}")
                pass
            
            exit_event_vo.set()
            
            T_tot = np.eye(4)
            trajectory = np.zeros((len(frame_data_list), 3, 4))
            trajectory[0] = T_tot[:3, :]
            
            for ix, posit in enumerate(vo_pos):
                trajectory[ix+1, :, :] = posit
                

            return entry,Data_list, frame_data_list, time_list, timelist_zed, (trajectory, vo_pos, vo_frame_time)
